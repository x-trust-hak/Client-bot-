...
// ── .take — rewrite a sticker's pack name/author metadata.
//    WhatsApp reads this from a WebP "EXIF" chunk holding a small JSON
//    blob, prefixed with a fixed fake-TIFF header so WhatsApp's parser
//    recognizes it. This exact header is the de-facto standard used by
//    essentially every WA sticker tool (wa-sticker-formatter and others
//    all use the same bytes) — node-webpmux just handles the RIFF
//    chunk read/write so we don't have to hand-patch the container. ──
async function setStickerExif(inputBuffer, packName, packPublisher) {
    const webpmux = require('node-webpmux');

    const json = {
        'sticker-pack-id': `liya-${Date.now()}`,
    ...
    img.exif = exif;
    return img.save(null); // null path -> returns a Buffer instead of writing to disk
}
