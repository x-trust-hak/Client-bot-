...
const DEFAULT_ECONOMY = {
    ...
    wins: 0,
    losses: 0,
    lastGift: 0,       // cooldown for .gift
    lastInterest: 0,   // cooldown for .interest
    loanAmount: 0,     // outstanding loan balance (principal + interest owed)
    loanDue: 0,        // timestamp the loan is due by
    stocks: '{}'       // JSON string: { SYMBOL: shares }
};
...
async function getProfile(redisClient, jid) {
    try {
        ...
        return {
            ...
            lastGift: parseInt(data.lastGift || '0'),
            lastInterest: parseInt(data.lastInterest || '0'),
            loanAmount: parseInt(data.loanAmount || '0'),
            loanDue: parseInt(data.loanDue || '0'),
            stocks: data.stocks || '{}'
        };
    } catch (err) {
...
//    Positive amounts get boosted by any active seasonal event multiplier;
//    negative amounts (bets, purchases, fines) are never boosted. ──
async function addCoins(redisClient, jid, amount) {
    let finalAmount = amount;
    if (amount > 0) {
        try {
            const multiplier = await events.getActiveMultiplier(redisClient, 'coins');
            finalAmount = Math.floor(amount * multiplier);
        } catch {
            // If the event lookup fails for any reason, fall back to the
            // unboosted amount rather than blocking the transaction.
            finalAmount = amount;
        }
    }

    const profile = await getProfile(redisClient, jid);
    const newBalance = Math.max(0, profile.coins + finalAmount);
...
async function addXp(redisClient, jid, amount) {
    // Apply active seasonal event multiplier first
    try {
        const eventMultiplier = await events.getActiveMultiplier(redisClient, 'xp');
        if (eventMultiplier !== 1) {
            amount = Math.floor(amount * eventMultiplier);
        }
    } catch {
        // Fall back to unboosted amount if the event lookup fails
    }

    // Apply active XP boost multiplier if one exists
    try {
        const boostRaw = await redisClient.hGet(`economy:${jid}`, 'xpBoost');
        if (boostRaw) {
            const boost = JSON.parse(boostRaw);
            if (Date.now() <= boost.expiresAt) {
                amount = Math.floor(amount * boost.multiplier);
            } else {
                // Boost expired — clean it up silently
                await redisClient.hDel(`economy:${jid}`, 'xpBoost');
            }
        }
        // Also apply pet XP bonus if one exists
        const petRaw = await redisClient.hGet(`economy:${jid}`, 'pet');
        if (petRaw) {
            const pet = JSON.parse(petRaw);
            amount = Math.floor(amount * (1 + pet.xpBonus));
        }
    } catch {}
...
// ════════════════════════════════════════════════════════
// BIRTHDAY SYSTEM
// Stored as birthday:<jid> = "DD/MM" string
// ════════════════════════════════════════════════════════

async function setBirthday(redisClient, jid, ddmm) {
    await redisClient.set(`birthday:${jid}`, ddmm);
}

async function getBirthday(redisClient, jid) {
    return redisClient.get(`birthday:${jid}`);
}

// Returns all JIDs whose birthday is today (DD/MM matching)
async function getTodayBirthdays(redisClient) {
    const today = new Date();
    const todayStr = `${String(today.getDate()).padStart(2, '0')}/${String(today.getMonth() + 1).padStart(2, '0')}`;

    // Scan all birthday:* keys — fine at personal-bot scale
    const keys = [];
    let cursor = 0;
    do {
        const result = await redisClient.scan(cursor, { MATCH: 'birthday:*', COUNT: 100 });
    ...
    for (const key of keys) {
        ...
        if (val === todayStr) matches.push(key.replace('birthday:', ''));
    }
    return matches;
}
...
module.exports.getTodayBirthdays = getTodayBirthdays;
