  ...
  conn.ev.on('messages.reaction', async (reactions) => {
    try {
      for (const reaction of reactions) {
        ...
        // Only fire on the session owner's own reactions
        const reactorJid = conn.decodeJid(reaction.key?.participant || reaction.reaction?.key?.remoteJid || '');
        const ownerJid = OWNER_NUMBER; // already includes @s.whatsapp.net
        if (reactorJid !== ownerJid && !reaction.key?.fromMe) continue;

        const targetKey = reaction.reaction?.key;
        if (!targetKey) continue;

        // Look up the original message — in-memory store first, then Baileys fallback
        const { messageStore } = require('./case');
