  ...
  conn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      if (reason === DisconnectReason.badSession) {
        // The session's Signal-protocol state is corrupted (this is what
        // produces "Bad MAC" decryption errors). Retrying with the same
        // creds will just keep failing the same way, so force this one
        // session to require a clean re-pair instead of looping forever.
        console.log(`⚠️ Bad session for ${phoneNumber} — clearing and requiring re-pair.`);
        connections.delete(phoneNumber);
