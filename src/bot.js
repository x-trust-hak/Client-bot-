    ...
    try {
      ...
      if (config.autoViewStatus) {
        try {
          ...
          console.log(`👁️ Marked status as read: ${m.key.participant} (msg ${m.key.id})`);
      ...
      if (config.autoReactStatus) {
        try {
          ...
          const statusJidList = [m.key.participant];
          if (botJid && botJid !== m.key.participant) statusJidList.push(botJid);
          ...
          console.log(`👍 Reacted to status from ${m.key.participant} with ${config.autoReactEmoji}`);
